...
describe('GET /transaction/balance', () => {
  ...
  it('should get transactions balance', async () => {
    ...
    (makeTransactionUseCases as jest.Mock).mockImplementation(() => ({
      balance: jest.fn().mockReturnValue(200),
    }));
    ...
    expect(transaction.body).toEqual({ balance: 200 });
  });
  ...
  it('should return 404 if error', async () => {
    ...
    (makeTransactionUseCases as jest.Mock).mockImplementation(() => ({
      balance: jest.fn().mockRejectedValueOnce(new Error('Balance not found')),
    }));
